...
const getCachedContributions = cache(async (userName, fromISO, toISO) => {
  ...
  const preparedContributions = prepareContributionData(
    rawContributions,
    from,
    to,
  );
...
function prepareContributionData(rawContributions, from, _to) {
  ...
  for (let i = 0; i < 17; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      ...
      const contribution = rawContributions.find(
        (c) => c.date === dateString,
      ) || { count: 0, date: dateString };
